import{_ as t,o,c as s,e as n,a as e,b as a}from"./app-BihAYnmf.js";const i={},r=n(`<blockquote><p>写在前面：Java SE5提供了一种新的类型-<a href="/archives/195" target="_blank">Java的枚举类型</a>，关键字enum可以将一组具名的值的有限集合创建为一种新的类型，而这些具名的值可以作为常规的程序组件使用，这是一种非常有用的功能。本文将深入分析枚举的源码，看一看枚举是怎么实现的，他是如何保证线程安全的，以及为什么用枚举实现的单例是最好的方式。</p></blockquote><h3 id="枚举是如何保证线程安全的" tabindex="-1"><a class="header-anchor" href="#枚举是如何保证线程安全的"><span>枚举是如何保证线程安全的</span></a></h3><p>要想看源码，首先得有一个类吧，那么枚举类型到底是什么类呢？是enum吗？答案很明显不是，enum就和class一样，只是一个关键字，他并不是一个类，那么枚举是由什么类维护的呢，我们简单的写一个枚举：</p><pre><code>public enum t {
...
</code></pre><p>都是static类型的，因为static类型的属性会在类被加载之后被初始化，我们在<a href="/archives/199" target="_blank">深度分析Java的ClassLoader机制（源码级别）</a>和<a href="/archives/201" target="_blank">Java类的加载、链接和初始化</a>两个文章中分别介绍过，当一个Java类第一次被真正使用到的时候静态资源被初始化、Java类的加载和初始化过程都是线程安全的。所以，<strong>创建一个enum类型是线程安全的</strong>。</p><h3 id="为什么用枚举实现的单例是最好的方式" tabindex="-1"><a class="header-anchor" href="#为什么用枚举实现的单例是最好的方式"><span>为什么用枚举实现的单例是最好的方式</span></a></h3><p>在<a href="/archives/205" target="_blank">[转+注]单例模式的七种写法</a>中，我们看到一共有七种实现单例的方式，其中，<strong>Effective Java</strong>作者<code>Josh Bloch</code> 提倡使用枚举的方式，既然大神说这种方式好，那我们就要知道它为什么好？</p><p><strong>1. 枚举写法简单</strong></p>`,13),c=e("blockquote",null,[e("p",null,[a("写法简单这个大家看看"),e("a",{hrerf:"/archives/205",target:"_blank"},"[转+注]单例模式的七种写法"),a("里面的实现就知道区别了。")])],-1),l=n(`<pre><code>public enum EasySingleton{
    INSTANCE;
}
...
</code></pre><p>从代码中可以看到，代码会尝试从调用<code>enumType</code>这个<code>Class</code>对象的<code>enumConstantDirectory()</code>方法返回的<code>map</code>中获取名字为<code>name</code>的枚举对象，如果不存在就会抛出异常。再进一步跟到<code>enumConstantDirectory()</code>方法，就会发现到最后会以反射的方式调用<code>enumType</code>这个类型的<code>values()</code>静态方法，也就是上面我们看到的编译器为我们创建的那个方法，然后用返回结果填充<code>enumType</code>这个<code>Class</code>对象中的<code>enumConstantDirectory</code>属性。</p><p>所以，<strong>JVM对序列化有保证。</strong></p><p><strong>3.枚举实例创建是thread-safe(线程安全的)</strong></p><blockquote><p>我们在<a href="/archives/199" target="_blank">深度分析Java的ClassLoader机制（源码级别）</a>和<a href="/archives/201" target="_blank">Java类的加载、链接和初始化</a>两个文章中分别介绍过，当一个Java类第一次被真正使用到的时候静态资源被初始化、Java类的加载和初始化过程都是线程安全的。所以，<strong>创建一个enum类型是线程安全的</strong>。</p></blockquote>`,9),u=[r,c,l];function d(p,m){return o(),s("div",null,u)}const b=t(i,[["render",d],["__file","enum-serializable.html.vue"]]),T=JSON.parse('{"path":"/docs/java/java-basic/enum-serializable.html","title":"","lang":"en-US","frontmatter":{},"headers":[{"level":3,"title":"枚举是如何保证线程安全的","slug":"枚举是如何保证线程安全的","link":"#枚举是如何保证线程安全的","children":[]},{"level":3,"title":"为什么用枚举实现的单例是最好的方式","slug":"为什么用枚举实现的单例是最好的方式","link":"#为什么用枚举实现的单例是最好的方式","children":[]}],"git":{"createdTime":1715384499000,"updatedTime":1715384499000,"contributors":[{"name":"dong","email":"dwx_job@163.com","commits":1}]},"filePathRelative":"docs/java/java-basic/enum-serializable.md"}');export{b as comp,T as data};
